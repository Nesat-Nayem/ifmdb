...
const loginController = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (user.role === 'vendor') {
            try {
                const { computeActiveServicesForVendor } = yield Promise.resolve().then(() => __importStar(require('../vendor/subscription.util')));
                const active = yield computeActiveServicesForVendor(user._id);
                user.vendorActiveServices = active;
                yield auth_model_1.User.findByIdAndUpdate(user._id, { vendorActiveServices: active });
            }
            catch (e) {
